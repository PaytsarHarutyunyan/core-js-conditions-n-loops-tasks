 ...
 *  3, 0, 3   => false
 */
function isIsoscelesTriangle(sideA, sideB, sideC) {
  if (!sideA || !sideB || !sideC) return false;
  const maxSide = getMaxNumber(sideA, sideB, sideC);
  if (sideA + sideB + sideC - maxSide < maxSide) return false;
...
function convertToRomanNumerals(num) {
  ...
  if (num < 1 || num > 39) {
    throw new Error('Invalid input. The number must be between 1 and 39.');
  }
  ...
  for (let i = romanSymbols.length - 1; i >= 0; i -= 1) {
    while (remainingValue >= romanValues[i]) {
      result += romanSymbols[i];
      remainingValue -= romanValues[i];
    }
  }
 ...
 */
function convertNumberToString(inputNumberStr) {
  const numberWords = {
    0: 'zero',
    1: 'one',
    2: 'two',
  ...
  let resultStr = '';
  const inputArray = [...`${inputNumberStr}`];
  ...
  for (let i = 0; i < inputArray.length; i += 1) {
    switch (inputArray[i]) {
      ...
      case ',':
        ...
        break;
      default:
        resultStr += numberWords[inputArray[i]];
        break;
    }
    resultStr += i + 1 === inputArray.length ? '' : ' ';
  }
